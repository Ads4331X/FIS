...
export function ImageManager({ refreshKey, viewMode, onStatsChange }: ImageManagerProps) {
  ...
  const selectedCategory = useMemo(
    () => FOLDERS.find((c) => (folder === ALL_CATEGORY_VALUE ? "" : folder) === c.folder),
    [folder],
  );
  ...
  const sectionTitle =
    viewMode === "category" ? "By Category" : "All Images";
  ...
  useEffect(() => {
    ...
    loadTimerRef.current = window.setTimeout(async () => {
      ...
      setLoadMsg("");
      try {
        const { images: fetchedImages, nextCursor: cursor } = await fetchImages(prefixes);
  ...
  useEffect(() => {
    const loadedBytes = images.reduce((sum, image) => sum + (image.bytes ?? 0), 0);
    onStatsChange?.({
  ...
  const loadNextChunk = async () => {
    ...
    const requestId = ++requestIdRef.current;
    try {
      const { images: moreImages, nextCursor: cursor } = await fetchImages(prefixes, nextCursor);
      if (requestId !== requestIdRef.current) return;
      setImages((prev) => [...prev, ...moreImages]);
      setNextCursor(cursor);
    } catch (e) {
      if (requestId !== requestIdRef.current) return;
      setLoadMsg(e instanceof Error ? e.message : "Could not load more images.");
    } finally {
          ...
          <FormControl size="small" sx={{ minWidth: { xs: "100%", sm: 260 } }}>
            ...
            >
              {FOLDERS.map((category) => (
                <MenuItem key={category.folder || ALL_CATEGORY_VALUE} value={category.folder || ALL_CATEGORY_VALUE}>
                  {category.label}
                </MenuItem>
              ))}
            </Select>
      ...
      {loading ? (
        ...
        <>
          <Typography sx={{ color: "text.secondary", fontSize: "0.85rem", mb: 2 }}>
            Showing {(page - 1) * PAGE_SIZE + 1}-{Math.min(page * PAGE_SIZE, sortedImages.length)} of {sortedImages.length} loaded asset{sortedImages.length !== 1 ? "s" : ""}
          </Typography>
          <Box
            sx={{
              display: "grid",
          ...
          >
            {visibleImages.map((image) => (
              ...
              >
                ...
                />
                <CardActions sx={{ p: 1.1, justifyContent: "space-between", alignItems: "center" }}>
                  <Typography sx={{ fontSize: "0.78rem", color: "text.secondary", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1, fontWeight: 800 }}>
          ...
          >
            <Pagination
              page={page}
              count={Math.max(nextCursor ? totalLoadedPages + 1 : totalLoadedPages, page)}
              color="primary"
              shape="rounded"
            ...
            <Typography sx={{ color: "text.secondary", fontSize: "0.82rem" }}>
              {nextCursor ? "More images available." : "End of loaded collection."}
